...
def create_new_image(config, existed_traits):
  ...
  for incomp in config["incompatibilities"]:
    for attr in new_image:
      if new_image[incomp["layer"]] == incomp["value"] and new_image[attr] in incomp["incompatible_with"]:
        print("[Traits Error]: {}, {} --> These traits are not allowed to be rendered togather! "
              "Finding legit new img >>> ...".format(new_image[incomp["layer"]], new_image[attr]))
  ...
  if encoded_traits not in existed_traits:
    ...
    print("Error: Repeated content, same as NFT #{}! Finding legit new img >>> ...".format(existed_traits[encoded_traits]))
    return create_new_image(config, existed_traits)

def gen_hybrid(tokenId_1, tokenId_2, config, prefix_path):
  print("Generating {} new unique NFTs...")
  ...
  print("Father's sterilization risk: {}, Mother's sterilization risk: {}".format(father_sterilization_risk,
                                                                                  mother_sterilization_risk))

  if risk < child_sterilization_risk:
    print("Random risk: {}".format(risk))
    print("[Hybrid Error]: Failed to get hybrid child! Try to get our insurance next time to get a guaranteed hybrid child.")
    return None
    # return "Hybrid Error: Failed to get hybrid child! Try to get our insurance next time to get a guaranteed hybrid child."
  print("Child's sterilization risk: {}".format(child_sterilization_risk))
  print("*"*30)
  ...
  print("Father's origin type: {}, Mother's origin type: {}".format(father_origin, mother_origin))
  print("Child's origin type: {}".format(origin))
  print("*" * 30)
  ...
  print("Hybrid Time: {}".format(waitTime))
  ...
  print("Check new NFT: {} in ./images!".format(file_name))
  # rgb_im.show("New NFT #{}".format(new_tokenId))
  return new_tokenId
